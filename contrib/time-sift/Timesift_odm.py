...
def main(argv):
    ...
    parser = argparse.ArgumentParser(
        formatter_class=argparse.RawDescriptionHelpFormatter,description=textwrap.dedent('''\
            Timesift_odm.py [-d <dataset-dir>] [-t <timesift-dir>] [-i <imageepochs-file>] [<options passed to ODM>]
    ...
    parser.add_argument('-t', '--timesift-dir', help='Time-SIFT directory') # must be in the datasets_DIR
    parser.add_argument('-i', '--imageepochs-file', help='Text file describing epochs') # must be in the timesift_DIR
    args, additional_options_to_rerun = parser.parse_known_args()
    if args.dataset_dir:
        datasets_DIR = Path(args.dataset_dir).absolute().as_posix()
    else:
        datasets_DIR = os.getenv("HOME")+'/datasets'
    ...
    def check_path_args(var: Path):
        if not var.exists():
            print(f'\033[91m[Time-SIFT]    ERROR: the {var.as_posix()} directory does not exist. Exiting program\033[0m')
            exit()
    ...
    ### Check for existing epochs directories before computing anything (these directories must be deleted)
    for epoch in images_epochs_dict:
        if Path(datasets_DIR,epoch).exists():
            print(f'\033[91m[Time-SIFT]    ERROR: {Path(datasets_DIR,epoch).as_posix()} already exists.')
            print(f'               => Consider deleting it. Exiting program\033[0m')
            exit()
    ...
    for epoch in images_epochs_dict:
        ...
        with open(Path(datasets_DIR,epoch,'opensfm','undistorted','reconstruction.json'), mode="r", encoding="utf-8") as read_file:
        ...
        with open(Path(datasets_DIR,epoch,'opensfm','undistorted','reconstruction.json'), mode="w", encoding="utf-8") as write_file:
