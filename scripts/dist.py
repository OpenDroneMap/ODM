...
This packages what `pixi run -e gpu build` (SuperBuild native binaries) and
`pixi install -e gpu-prod` (the GPU runtime conda environment at
.pixi/envs/gpu-prod) produce into an Inno Setup installer. This script only does
the packaging (download Inno Setup + the VC++ redistributable, optionally
code-sign, then run the Inno Setup compiler).
"""
import argparse
import os
import shutil
...
def main():
    ...
    if not os.path.isdir(os.path.join(ROOT, "SuperBuild", "install")):
        sys.exit("SuperBuild/install not found. Run `pixi run -e gpu build` first.")
    if not os.path.isdir(os.path.join(ROOT, ".pixi", "envs", "gpu-prod")):
        sys.exit(".pixi/envs/gpu-prod not found. Run `pixi install --locked -e gpu-prod` first.")
