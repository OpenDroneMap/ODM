...
class TestOSFM(unittest.TestCase):
    ...
    def test_get_submodel_argv(self):
        ...
        self.assertEqual(
            get_submodel_argv(args, "/submodels", "submodel_0000")[1:],
            [
                "--orthophoto-cutline",
                "--dem-euclidean-map",
                "--skip-3dmodel",
        ...
        args = config.config(
            ["--project-path", "/datasets", "--split", "200", "--pc-csv"]
        )
        self.assertEqual(
            get_submodel_argv(args)[1:],
            ["--orthophoto-cutline", "--dem-euclidean-map", "--skip-3dmodel"],
        ...
        args = config.config(
            [
                ...
                "--cameras",
                os.path.join(
                    os.path.dirname(os.path.realpath(__file__)), "assets", "sample.json"
                ),
            ]
        )
        self.assertEqual(
        ...
        # Camera JSON string
        args = config.config(
            ["--project-path", "/datasets", "--cameras", '{"test": "1"}']
        )
        self.assertEqual(
            get_submodel_argv(args)[1:],
            [
