...
class ODMGeoreferencingStage(types.ODM_Stage):
    def process(self, args, outputs):
        ...
        gcp_gml_export_file = tree.path(
            "odm_georeferencing", "ground_control_points.gml"
        )
        gcp_geojson_export_file = tree.path(
            "odm_georeferencing", "ground_control_points.geojson"
        )
        gcp_geojson_zip_export_file = tree.path(
            "odm_georeferencing", "ground_control_points.zip"
        )
        unaligned_model = io.related_file_path(
            tree.odm_georeferencing_model_laz, postfix="_unaligned"
        )
        if os.path.isfile(unaligned_model) and self.rerun():
            os.unlink(unaligned_model)

        if reconstruction.has_gcp() and (
            ...
            if len(gcps):
                ...
                try:
                    system.run(
                        'ogr2ogr -of GML "{}" "{}"'.format(
                            gcp_gml_export_file, gcp_export_file
                        )
                    )
                except Exception as e:
                    log.ODM_WARNING(
                        "Cannot generate ground control points GML file: %s" % str(e)
                    )
                    ...
                    geojson["features"].append(
                        {
                            ...
                            "geometry": {
                                "type": "Point",
                                "coordinates": transformer.TransformPoint(
                                    *gcp["coordinates"]
                                ),
                            },
                ...
                with zipfile.ZipFile(
                    gcp_geojson_zip_export_file, "w", compression=zipfile.ZIP_LZMA
                ) as f:
                    f.write(
                ...
                log.ODM_WARNING(
                    "GCPs could not be loaded for writing to %s" % gcp_export_file
                )

        if not io.file_exists(tree.odm_georeferencing_model_laz) or self.rerun():
            cmd = f'pdal translate -i "{tree.filtered_point_cloud}" -o "{tree.odm_georeferencing_model_laz}"'
            stages = ["ferry"]
            params = ['--filters.ferry.dimensions="views => UserData"']
            ...
            if reconstruction.is_georeferenced():
                ...
                las_scale = 0.001
                filtered_point_cloud_stats = tree.path(
                    "odm_filterpoints", "point_cloud_stats.json"
                )
                ...
                if os.path.isfile(filtered_point_cloud_stats):
                    try:
                        with open(filtered_point_cloud_stats, "r") as stats:
                            ...
                            log.ODM_INFO(
                                "las scale calculated as the minimum of 1/10 estimated spacing or %s, which ever is less."
                                % las_scale
                            )
                            las_scale = min(spacing, 0.001)
                    except Exception as e:
                        log.ODM_WARNING(
                            "Cannot find file point_cloud_stats.json. Using default las scale: %s"
                            % las_scale
                        )
                else:
                    log.ODM_INFO(
                        "No point_cloud_stats.json found. Using default las scale: %s"
                        % las_scale
                    )
                ...
                if reconstruction.has_gcp() and io.file_exists(
                    gcp_geojson_zip_export_file
                ):
                    if os.path.getsize(gcp_geojson_zip_export_file) <= 65535:
                        log.ODM_INFO("Embedding GCP info in point cloud")
                        params += [
                            '--writers.las.vlrs="{\\"filename\\": \\"%s\\", \\"user_id\\": \\"ODM\\", \\"record_id\\": 2, \\"description\\": \\"Ground Control Points (zip)\\"}"'
                            % gcp_geojson_zip_export_file.replace(os.sep, "/")
                        ]
                    else:
                        log.ODM_WARNING(
                            "Cannot embed GCP info in point cloud, %s is too large"
                            % gcp_geojson_zip_export_file
                        )
                ...
                if args.crop > 0:
                    ...
                    cropper = Cropper(
                        tree.odm_georeferencing, "odm_georeferenced_model"
                    )
            ...
            if tree.odm_align_file is not None:
                alignment_file_exists = io.file_exists(
                    tree.odm_georeferencing_alignment_matrix
                )
                ...
                if not alignment_file_exists or self.rerun():
                    ...
                    a_matrix = None
                    try:
                        a_matrix = compute_alignment_matrix(
                            tree.odm_georeferencing_model_laz,
                            tree.odm_align_file,
                            stats_dir,
                        )
                    ...
                    if a_matrix is not None:
                        ...
                        if os.path.isfile(unaligned_model):
                            os.rename(
                                unaligned_model, tree.odm_georeferencing_model_laz
                            )
                        ...
                        try:
                            ...
                            log.ODM_INFO(
                                "Transformed %s" % tree.odm_georeferencing_model_laz
                            )
                        except Exception as e:
                        ...
                        def transform_textured_model(obj):
                            if os.path.isfile(obj):
                                ...
                                try:
                                    ...
                                    log.ODM_WARNING(
                                        "Cannot transform textured model: %s" % str(e)
                                    )
                                    os.rename(unaligned_obj, obj)
                        ...
                        for texturing in [tree.odm_texturing, tree.odm_25dtexturing]:
                            if reconstruction.multi_camera:
                                ...
                                for band in reconstruction.multi_camera:
                                    subdir = (
                                        ""
                                        if band["name"] == primary
                                        else band["name"].lower()
                                    )
                                    obj = os.path.join(
                                        texturing, subdir, "odm_textured_model_geo.obj"
                                    )
                                    transform_textured_model(obj)
                            else:
                                obj = os.path.join(
                                    texturing, "odm_textured_model_geo.obj"
                                )
                                transform_textured_model(obj)
                        ...
                        log.ODM_WARNING(
                            "Alignment to %s will be skipped." % tree.odm_align_file
                        )
                else:
            ...
            log.ODM_WARNING(
                "Found a valid georeferenced model in: %s"
                % tree.odm_georeferencing_model_laz
            )

        if (
            args.optimize_disk_space
            and io.file_exists(tree.odm_georeferencing_model_laz)
            and io.file_exists(tree.filtered_point_cloud)
        ):
