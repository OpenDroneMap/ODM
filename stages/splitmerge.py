...
from opendm.osfm import (
    OSFMContext,
    get_submodel_argv,
    get_submodel_paths,
    get_all_submodel_paths,
)
    ...
    def process(self, args, outputs):
        ...
        if io.file_exists(image_groups_file):
            ...
            if reconstruction.has_geotagged_photos():
                ...
                log.ODM_WARNING(
                    "Could not perform split-merge as GPS information in photos or image_groups.txt is missing."
                )
        ...
        if outputs["large"]:
            ...
            if not io.file_exists(split_done_file) or self.rerun():
                ...
                multiplier = (
                    (1.0 / len(reconstruction.multi_camera))
                ...
                config = [
                    ...
                    + os.path.join("..", "submodels", "submodel_%04d", "opensfm"),
                ...
                if not io.dir_exists(tree.submodels_path) or self.rerun():
                    ...
                    log.ODM_WARNING(
                        "Submodels directory already exist at: %s" % tree.submodels_path
                    )
                ...
                for sp in submodel_paths:
                    ...
                    if reconstruction.gcp and reconstruction.gcp.exists():
                        ...
                        if reconstruction.gcp.make_filtered_copy(
                            ...
                            log.ODM_INFO(
                                "Copied filtered GCP file to %s" % submodel_gcp_file
                            )
                            io.copy(
                                submodel_gcp_file,
                    ...
                    if reconstruction.multi_camera:
                        ...
                        _, p2s = multispectral.compute_band_maps(
                        ...
                        for filename in p2s:
                            if filename in submodel_images:
                                ...
                                for p in secondary_band_photos:
                                    system.link_file(
                                        os.path.join(tree.dataset_raw, p.filename),
                                        submodel_images_dir,
                                    )
                ...
                if local_workflow:
                    for sp in submodel_paths:
                        ...
                        local_sp_octx.reconstruct(
                            args.rolling_shutter, not args.sfm_no_partial, self.rerun()
                        )
                else:
                    lre = LocalRemoteExecutor(
                        args.sm_cluster, args.rolling_shutter, self.rerun()
                    )
                    lre.set_projects(
                        [os.path.abspath(os.path.join(p, "..")) for p in submodel_paths]
                    )
                ...
                if not args.sm_no_align:
                    ...
                    for sp in submodel_paths:
                        ...
                        if (
                            io.file_exists(main_recon)
                            and io.file_exists(unaligned_recon)
                            and not self.rerun()
                        ):
                            log.ODM_INFO(
                                "Submodel %s has already been aligned." % sp_octx.name()
                            )
                            continue
                ...
                if local_workflow:
                    for sp in submodel_paths:
                        ...
                        argv = get_submodel_argv(
                            args, tree.submodels_path, sp_octx.name()
                        )

                        # Re-run the ODM toolchain on the submodel
                        system.run(
                            " ".join(map(double_quote, map(str, argv))),
        ...
        if outputs["large"]:
            ...
            if args.merge in ["all", "pointcloud"]:
                if (
                    ...
                    try:
                        ...
                        log.ODM_WARNING(
                            "Could not merge point cloud: %s (skipping)" % str(e)
                        )
                else:
                    log.ODM_WARNING(
                        "Found merged point cloud in %s"
                        % tree.odm_georeferencing_model_laz
                    )
            ...
            if args.merge in ["all", "orthophoto"]:
                ...
                if not io.file_exists(tree.odm_orthophoto_tif) or self.rerun():
                    ...
                    if len(all_orthos_and_ortho_cuts) > 1:
                        ...
                        shutil.copyfile(
                            all_orthos_and_ortho_cuts[0][0], tree.odm_orthophoto_tif
                        )
                    else:
                        log.ODM_WARNING(
                    ...
                    log.ODM_WARNING(
                        "Found merged orthophoto in %s" % tree.odm_orthophoto_tif
                    )
            ...
            def merge_dems(dem_filename, human_name):
                ...
                if not io.file_exists(dem_file) or self.rerun():
                    all_dems = get_submodel_paths(
                        tree.submodels_path, "odm_dem", dem_filename
                    )
                    ...
                    euclidean_merge_dems(
                        all_dems, dem_file, dem_vars, euclidean_map_source=eu_map_source
                    )
                    ...
                    if io.file_exists(dem_file):
                        ...
                        log.ODM_WARNING(
                            "Cannot merge %s, %s was not created"
                            % (human_name, dem_file)
                        )

                else:
                    log.ODM_WARNING(
                        "Found merged %s in %s" % (human_name, dem_filename)
                    )
            ...
            if not io.file_exists(geojson_shots) or self.rerun():
                ...
                log.ODM_INFO(
                    "Merging %s shots.geojson files" % len(geojson_shots_files)
                )
            ...
            if not io.file_exists(cameras_json) or self.rerun():
                cameras_json_files = get_submodel_paths(
                    tree.submodels_path, "cameras.json"
                )
