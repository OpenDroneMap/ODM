...
class ODMReport(types.ODM_Stage):
    def process(self, args, outputs):
        ...
        if not os.path.exists(tree.odm_report):
        ...
        if not io.file_exists(shots_geojson) or self.rerun():
            ...
            if reconstruction.is_georeferenced():
                ...
                # Pseudo geo
                shots = get_geojson_shots_from_opensfm(
                    tree.opensfm_reconstruction, pseudo_geotiff=tree.odm_orthophoto_tif
                )
        ...
        if not io.file_exists(camera_mappings) or self.rerun():
            ...
            log.ODM_WARNING(
                "Found a valid camera mappings file in: %s" % camera_mappings
            )
        ...
        if (
            odm_stats.get("point_cloud_statistics")
            and point_cloud_file
            and views_dimension
        ):
            bounds = (
                odm_stats["point_cloud_statistics"]
            ...
            if bounds:
                ...
                report_assets = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), "../opendm/report")
                )
                ...
                if (args.crop > 0 or args.boundary) and os.path.isfile(
                    bounds_file_path
                ):
                    Cropper.crop(
                        bounds_file_path,
                ...
                system.run(
                    'gdaldem color-relief "{}" "{}" "{}" -of PNG -alpha'.format(
                        diagram_tiff, overlap_color_map, diagram_png
                    )
                )
                ...
                for asset in ["overlap_diagram_legend.png", "dsm_gradient.png"]:
                    shutil.copy(
                        os.path.join(report_assets, asset),
                ...
                for dem in dems:
                    ...
                    if os.path.isfile(dem_file):
                        ...
                        system.run(
                            'gdal_translate -outsize {} 0 "{}" "{}" --config GDAL_CACHEMAX {}%'.format(
                                image_target_size,
                                dem_file,
                                resized_dem_file,
                                get_max_memory(),
                        ...
                        system.run(
                            'gdal_translate -outsize {} 0 -of png "{}" "{}" --config GDAL_CACHEMAX {}%'.format(
                                image_target_size,
