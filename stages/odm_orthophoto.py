    ...
    def process(self, args, outputs):
        ...
        if not io.file_exists(tree.odm_orthophoto_tif) or self.rerun():
            ...
            kwargs = {
                ...
                "a_srs": "",
                "vars": "",
                "gdal_configs": "--config GDAL_CACHEMAX %s"
                % (get_max_memory_mb() * 1024 * 1024),
            }
            ...
            if reconstruction.multi_camera:
                ...
                kwargs["bands"] = "-bands %s" % (
                    ",".join(
                        [double_quote(b["name"]) for b in reconstruction.multi_camera]
                    )
                )
            ...
            if reconstruction.is_georeferenced():
                ...
                kwargs["utm_offsets"] = "-utm_north_offset %s -utm_east_offset %s" % (
                ...
                kwargs["vars"] = " ".join(
                    ["-co %s=%s" % (k, orthophoto_vars[k]) for k in orthophoto_vars]
                )
                kwargs["ortho"] = (
                    tree.odm_orthophoto_tif
                )  # Render directly to final file
            ...
            system.run(
                ...
                "{utm_offsets} {a_srs} {vars} {gdal_configs} ".format(**kwargs),
            ...
            if reconstruction.is_georeferenced():
                ...
                if args.orthophoto_cutline:
                    ...
                    if submodel_run:
                        ...
                        log.ODM_INFO(
                            "Not a submodel run, skipping mask raster generation"
                        )
                ...
                if args.orthophoto_cutline and submodel_run:
                    orthophoto.feather_raster(
                        tree.odm_orthophoto_tif,
                        os.path.join(
                            tree.odm_orthophoto, "odm_orthophoto_feathered.tif"
                        ),
                        blend_distance=20,
                    )
