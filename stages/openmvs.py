    ...
    def process(self, args, outputs):
        ...
        if not photos:
            raise system.ExitException(
                "Not enough photos in photos array to start OpenMVS"
            )
        ...
        if not io.file_exists(tree.openmvs_model) or self.rerun():
            ...
            try:
                ...
                if e.errorCode == 1 and use_gpu:
                    log.ODM_WARNING(
                        "OpenMVS failed with GPU, is your graphics card driver up to date? Falling back to CPU."
                    )
                    ...
                    e.errorCode == 137
                    or e.errorCode == 143
                    or e.errorCode == 3221226505
                ) and not pc_tile:
                    log.ODM_WARNING(
                        "OpenMVS ran out of memory, we're going to turn on tiling to see if we can process this."
                    )
                    pc_tile = True
            ...
            if pc_tile:
                ...
                subscene_densify_ini_file = os.path.join(
                    tree.openmvs, "subscene-config.ini"
                )
                ...
                scene_files = glob.glob(
                    os.path.join(tree.openmvs, "scene_[0-9][0-9][0-9][0-9].mvs")
                )
                ...
                for sf in scene_files:
                    ...
                    if not io.file_exists(scene_ply) or self.rerun():
                        ...
                        if not io.file_exists(scene_ply_unfiltered):
                            ...
                            if args.pc_filter > 0:
                                system.run(
                                    '"%s" "%s" --filter-point-cloud %s -v 0 --archive-type 3 %s'
                                    % (
                                        context.omvs_densify_path,
                                        scene_dense_mvs,
                                        filter_point_th,
                                ...
                                # Just rename
                                log.ODM_INFO(
                                    "Skipped filtering, %s --> %s"
                                    % (scene_ply_unfiltered, scene_ply)
                                )
                                os.rename(scene_ply_unfiltered, scene_ply)
                    else:
                        log.ODM_WARNING(
                            "Found existing dense scene file %s" % scene_ply
                        )
                ...
                def skip_filtering():
                    ...
                    if not os.path.exists(scene_dense_ply):
                        raise system.ExitException(
                            "Dense reconstruction failed. This could be due to poor georeferencing or insufficient image overlap."
                        )

                    log.ODM_INFO(
                        "Skipped filtering, %s --> %s"
                        % (scene_dense_ply, tree.openmvs_model)
                    )
                    ...
                    if os.path.exists(scene_dense):
                        ...
                        try:
                            ...
                            if (
                                ...
                                log.ODM_WARNING(
                                    "OpenMVS filtering ran out of memory, visibility checks will be skipped."
                                )
                                skip_filtering()
                            else:
                                raise e
            ...
            log.ODM_WARNING(
                "Found a valid OpenMVS reconstruction file in: %s" % tree.openmvs_model
            )
