...
void OdmOrthoPhoto::createOrthoPhoto()
{
    ...
    try{
        ...
        std::cerr << "Couldn't allocate enough memory to render the orthophoto (" << colRes << "x" << rowRes << " cells = " << ((long long)colRes * (long long)rowRes * 4) << " bytes). Try to increase the --orthophoto-resolution to a larger integer or add more RAM.\n";
        exit(1);
    }
