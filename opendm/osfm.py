...
class OSFMContext:
    ...
    def setup(
        self, args, images_path, photos, gcp_path=None, append_config=[], rerun=False
    ):
        """
        ...
        if not io.file_exists(list_path) or rerun:
            ...
            if io.file_exists(image_groups_file):
                ...
                io.copy(
                    image_groups_file,
                    os.path.join(self.opensfm_project_path, "image_groups.txt"),
                )
            ...
            config = [
                ...
                % ("no" if args.use_fixed_camera_params or args.cameras else "yes"),
