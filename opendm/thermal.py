...
def dn_to_temperature(photo, image):
    ...
    if photo.is_thermal():
        ...
        if photo.camera_make == "MicaSense" and photo.camera_model == "Altum":
            ...
            log.ODM_WARNING("Unsupported camera [%s %s], thermal band will have digital numbers."
                            % (photo.camera_make, photo.camera_model))
    else:
        log.ODM_WARNING("Tried to radiometrically calibrate a non-thermal image with temperature values (%s)"
                        % photo.filename)
        return image
