...
def url_string(string):
    ...
    if re.match(regex, string) is None:
        raise argparse.ArgumentTypeError("%s is not a valid URL. The URL must be in the format: http(s)://host["
                                         ":port]/[?token=]" % string)
    return string
...
def config(argv=None, parser=None):
    ...
    if parser is None:
        parser = SettingsParser(
            description='ODM is a command line toolkit to generate maps, point clouds, 3D models and DEMs from drone, '
                        'balloon or kite images.',
    ...
    parser.add_argument(
        ...
        metavar='<path>',
        action=StoreValue,
        help='Path to the project folder. Your project folder should contain subfolders for each dataset. Each '
             'dataset should have an "images" folder.')
    parser.add_argument(
        'name',
    ...
    parser.add_argument(
        ...
        default=2048,
        type=int,
        help='Legacy option (use --feature-quality instead). Resizes images by the largest side for feature '
    ...
    parser.add_argument(
        ...
        help='Set feature extraction quality. Higher quality generates better features, but requires more memory and '
             'takes longer. '
             'Can be one of: %(choices)s. Default: '
             '%(default)s')
    ...
    parser.add_argument(
        ...
        default='flann',
        choices=['flann', 'bow'],
        help='Matcher algorithm, Fast Library for Approximate Nearest Neighbors or Bag of Words. FLANN is slower, '
             'but more stable. BOW is faster, but can sometimes miss valid matches. '
             'Can be one of: %(choices)s. Default: '
             '%(default)s')
    ...
    parser.add_argument(
        ...
        nargs=0,
        default=False,
        help='Turn off camera parameter optimization during bundle adjustment. This can be sometimes useful for '
             'improving results that exhibit doming/bowling or when images are taken with a rolling shutter camera. '
             'Default: %(default)s')
    ...
    parser.add_argument(
        ...
        help='Set the radiometric calibration to perform on images. '
             ...
             '[camera] applies black level, vignetting, row gradient gain/exposure compensation (if appropriate EXIF '
             'tags are found) and computes absolute temperature values. '
             '[camera+sun] is experimental, applies all the corrections of [camera], plus compensates for spectral '
             'radiance registered via a downwelling light sensor (DLS) taking in consideration the angle of the sun. '
             'Can be one of: %(choices)s. Default: '
             '%(default)s')
    ...
    parser.add_argument(
        ...
        type=float,
        default=640,
        help='Legacy option (use --pc-quality instead). Controls the density of the point cloud by setting the '
             'resolution of the depthmap images. Higher values take longer to compute '
    ...
    parser.add_argument(
        ...
        nargs=0,
        default=False,
        help='Use a full 3D mesh to compute the orthophoto instead of a 2.5D mesh. This option is a bit faster and '
             'provides similar results in planar areas. Default: %(default)s')
    ...
    parser.add_argument(
        ...
        nargs=0,
        default=False,
        help='Skip generation of a full 3D model. This can save time if you only need 2D results such as orthophotos '
             'and DEMs. Default: %(default)s')
    ...
    parser.add_argument(
        ...
        help='Ignore Ground Sampling Distance (GSD). GSD '
             ...
             'lower memory usage. Since GSD is an estimate, sometimes ignoring it can result in slightly better image '
             'output quality. Default: %(default)s')
    ...
    parser.add_argument(
        ...
        default='medium',
        choices=['ultra', 'high', 'medium', 'low', 'lowest'],
        help='Set point cloud quality. Higher quality generates better, denser point clouds, but requires more memory '
             'and takes longer. Each step up in quality increases processing time roughly by a factor of 4x. '
             'Can be one of: %(choices)s. Default: '
             '%(default)s')
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.5,
        help='Filters the point cloud by removing points that deviate more than N standard deviations from the local '
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help='Filters the point cloud by keeping only a single point around a radius N (in meters). This can be '
             'useful to limit the output resolution of the point cloud and remove duplicate points. Set to 0 to '
             'disable sampling. '
             'Default: %(default)s')
    ...
    parser.add_argument(
        ...
        nargs=0,
        default=False,
        help='Reduce the memory usage needed for depthmap fusion by splitting large scenes into tiles. Turn this on '
             'if your machine doesn\'t have much RAM and/or you\'ve set --pc-quality to high or ultra. Experimental. '
             'Default: %(default)s')
    ...
    parser.add_argument(
        ...
        nargs=0,
        default=False,
        help='Improve the accuracy of the point cloud by computing geometrically consistent depthmaps. This increases '
             'processing time, but can improve results in urban scenes. '
             'Default: %(default)s')
    ...
    parser.add_argument(
        ...
        default='gmi',
        choices=['gmi', 'area'],
        help='When texturing the 3D mesh, for each triangle, choose to prioritize images with sharp features (gmi) or '
             'those that cover the largest area (area). Default: %(default)s')
    ...
    parser.add_argument(
        ...
        nargs=0,
        default=False,
        help='Skip normalization of colors across all images. Useful when processing radiometric data. Default: %('
             'default)s')
    ...
    parser.add_argument(
        ...
        help='Path to the image geolocation file containing the camera center coordinates used for georeferencing. '
             ...
             'EPSG:<code> or <+proj definition>\n'
             'image_name geo_x geo_y geo_z [omega (degrees)] [phi (degrees)] [kappa (degrees)] [horz accuracy ('
             'meters)] [vert accuracy (meters)]\n '
             'Default: %(default)s')
    ...
    parser.add_argument(
        ...
        type=float,
        default=5,
        help='DSM/DTM resolution in cm / pixel. Note that this value is capped by a ground sampling distance (GSD) '
    ...
    parser.add_argument(
        ...
        help='Decimate the points before generating the DEM. 1 is no decimation (full quality). '
             '100 decimates ~99%% of the points. Useful for speeding up generation of DEM results in very large '
    ...
    parser.add_argument(
        ...
        default=5,
        type=float,
        help='Orthophoto resolution in cm / pixel. Note that this value is capped by a ground sampling distance (GSD) '
             'estimate. To remove the cap, check --ignore-gsd also. '
             'Default: %(default)s')
    ...
    parser.add_argument(
        ...
        help='Set a value in meters for the GPS Dilution of Precision (DOP) '
             ...
             'fails. Lowering this option can sometimes help control bowling-effects over large areas. '
             'Default: %(default)s')
    ...
    parser.add_argument(
        ...
        default="auto",
        type=str,
        help='When processing multispectral datasets, you can specify the name of the primary band that will be used '
             'for reconstruction. It\'s recommended to choose a band which has sharp details and is in focus. ' 
             'Default: %(default)s')
    ...
    #     log.ODM_WARNING("radiometric-calibration is turned on, automatically setting "
    #                     "--texturing-skip-global-seam-leveling")
