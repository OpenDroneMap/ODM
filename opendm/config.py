...
def config(argv=None, parser=None):
    ...
    parser.add_argument('--use-3dmesh',
                    ...
                    nargs=0,
                    default=False,
                    help='Use a full 3D mesh to compute the orthophoto instead of a 2.5D mesh. This option is a bit faster and provides similar results in planar areas. (If rerunning, restart "From Orthophoto/odm_orthophoto") Default: %(default)s')
    ...
    parser.add_argument('--mesh-size',
                        ...
                        default=200000,
                        type=int,
                        help=('The maximum vertex count of the output mesh. (If rerunning, restart "From Meshing/odm_meshing"). '
                              'Default: %(default)s'))
    ...
    parser.add_argument('--pc-quality',
                    ...
                    default='medium',
                    choices=['ultra', 'high', 'medium', 'low', 'lowest'],
                    help=('Set point cloud quality. Higher quality generates better, denser point clouds, but requires more memory and takes longer. Each step up in quality increases processing time roughly by a factor of 4x. (If rerunning, restart "From Multi View Stereo/openmvs"). '
                        'Can be one of: %(choices)s. Default: '
                        '%(default)s'))
    ...
    parser.add_argument('--pc-classify',
            ...
            'You can control the behavior of this option by tweaking the --dem-* parameters. (If rerunning, restart "From DEM/odm_dem"). '
            'Default: '
            '%(default)s')
    ...
    parser.add_argument('--pc-csv',
                        ...
                        nargs=0,
                        default=False,
                        help='Export the georeferenced point cloud in CSV format. (If rerunning, restart "From DEM/odm_dem"). Default: %(default)s')
    
    parser.add_argument('--pc-las',
                action=StoreTrue,
                nargs=0,
                default=False,
                help='Export the georeferenced point cloud in LAS format. (If rerunning, restart "From DEM/odm_dem"). Default: %(default)s')

    parser.add_argument('--pc-ept',
                action=StoreTrue,
                nargs=0,
                default=False,
                help='Export the georeferenced point cloud in Entwine Point Tile (EPT) format. (If rerunning, restart "From DEM/odm_dem"). Default: %(default)s')

    parser.add_argument('--pc-copc',
                action=StoreTrue,
                nargs=0,
                default=False,
                help='Save the georeferenced point cloud in Cloud Optimized Point Cloud (COPC) format. (If rerunning, restart "From DEM/odm_dem"). Default: %(default)s')
    ...
    parser.add_argument('--pc-filter',
                        ...
                        type=float,
                        default=2.5,
                        help='Filters the point cloud by removing points that deviate more than N standard deviations from the local mean. Set to 0 to disable filtering. (If rerunning, restart "From Point Filtering/odm_filterpoints"). '
                             'Default: %(default)s')
    ...
    parser.add_argument('--pc-sample',
                        ...
                        type=float,
                        default=0,
                        help='Filters the point cloud by keeping only a single point around a radius N (in meters). This can be useful to limit the output resolution of the point cloud and remove duplicate points. Set to 0 to disable sampling. (If rerunning, restart "From Point Filtering/odm_filterpoints"). '
                             'Default: %(default)s')
    ...
    parser.add_argument('--pc-skip-geometric',
                        ...
                        nargs=0,
                        default=False,
                        help='Geometric estimates improve the accuracy of the point cloud by computing geometrically consistent depthmaps but may not be usable in larger datasets. This flag disables geometric estimates. (If rerunning, restart "From Point Filtering/odm_filterpoints"). '
                             'Default: %(default)s')
    ...
    parser.add_argument('--smrf-scalar',
                        ...
                        type=float,
                        default=1.25,
                        help='Simple Morphological Filter elevation scalar parameter. (If rerunning, restart "From DEM/odm_dem"). '
                             'Default: %(default)s')
    ...
    parser.add_argument('--smrf-slope',
        ...
        type=float,
        default=0.15,
        help='Simple Morphological Filter slope parameter (rise over run). (If rerunning, restart "From DEM/odm_dem"). '
                'Default: %(default)s')
    ...
    parser.add_argument('--smrf-threshold',
        ...
        type=float,
        default=0.5,
        help='Simple Morphological Filter elevation threshold parameter (meters). (If rerunning, restart "From DEM/odm_dem"). '
                'Default: %(default)s')
    ...
    parser.add_argument('--smrf-window',
        ...
        type=float,
        default=18.0,
        help='Simple Morphological Filter window radius parameter (meters). (If rerunning, restart "From DEM/odm_dem"). '
                'Default: %(default)s')
    ...
    parser.add_argument('--texturing-skip-global-seam-leveling',
                        ...
                        nargs=0,
                        default=False,
                        help=('Skip normalization of colors across all images. Useful when processing radiometric data. (If rerunning, restart "From Texturing/mvs_texturing"). Default: %(default)s'))

    parser.add_argument('--texturing-skip-local-seam-leveling',
                        action=StoreTrue,
                        nargs=0,
                        default=False,
                        help='Skip the blending of colors near seams. (If rerunning, restart "From Texturing/mvs_texturing"). Default: %(default)s')

    parser.add_argument('--texturing-keep-unseen-faces',
                        action=StoreTrue,
                        nargs=0,
                        default=False,
                        help=('Keep faces in the mesh that are not seen in any camera. (If rerunning, restart "From Texturing/mvs_texturing"). '
                              'Default:  %(default)s'))
    ...
    parser.add_argument('--texturing-single-material',
                        ...
                        nargs=0,
                        default=False,
                        help=('Generate OBJs that have a single material and a single texture file instead of multiple ones. (If rerunning, restart "From Texturing/mvs_texturing"). '
                              'Default:  %(default)s'))
    ...
    parser.add_argument('--gltf',
                        ...
                        nargs=0,
                        default=False,
                        help=('Generate single file Binary glTF (GLB) textured models. (If rerunning, restart "From Texturing/mvs_texturing"). '
                              'Default:  %(default)s'))
    ...
    parser.add_argument('--dtm',
                        ...
                        help='Use this tag to build a DTM (Digital Terrain Model, ground only) using a simple '
                             'morphological filter. (If rerunning, restart "From DEM/odm_dem"). Check the --dem* and --smrf* parameters for finer tuning. Default: %(default)s')
    ...
    parser.add_argument('--dsm',
                        ...
                        help='Use this tag to build a DSM (Digital Surface Model, ground + objects) using a progressive '
                             'morphological filter. Check the --dem* parameters for finer tuning. (If rerunning, restart "From DEM/odm_dem"). Default: %(default)s')
    ...
    parser.add_argument('--dem-gapfill-steps',
                        ...
                        help='Number of steps used to fill areas with gaps. Set to 0 to disable gap filling. '
                             ...
                             'and merged together. Remaining gaps are then merged using nearest neighbor interpolation. (If rerunning, restart "From DEM/odm_dem"). '
                             'Default: %(default)s')
    ...
    parser.add_argument('--dem-resolution',
                        ...
                        type=float,
                        default=5,
                        help='DSM/DTM resolution in cm / pixel. Note that this value is capped by a ground sampling distance (GSD) estimate. (If rerunning, restart "From DEM/odm_dem"). '
                             ' Default: %(default)s')
    ...
    parser.add_argument('--dem-decimation',
                        ...
                        help='Decimate the points before generating the DEM. 1 is no decimation (full quality). '
                             '100 decimates ~99%% of the points. Useful for speeding up generation of DEM results in very large datasets. (If rerunning, restart "From DEM/odm_dem"). Default: %(default)s')
    ...
    parser.add_argument('--dem-euclidean-map',
            ...
            'This can be useful to isolate the areas that have been filled. (If rerunning, restart "From DEM/odm_dem"). '
            'Default: '
            '%(default)s')
    ...
    parser.add_argument('--orthophoto-resolution',
                        ...
                        default=5,
                        type=float,
                        help=('Orthophoto resolution in cm / pixel. Note that this value is capped by a ground sampling distance (GSD) estimate. (If rerunning, restart "From Orthophoto/odm_orthophoto"). '
                              'Default: %(default)s'))
    ...
    parser.add_argument('--orthophoto-no-tiled',
                        ...
                        nargs=0,
                        default=False,
                        help='Set this parameter if you want a striped GeoTIFF. (If rerunning, restart "From Orthophoto/odm_orthophoto"). '
                             'Default: %(default)s')
    ...
    parser.add_argument('--orthophoto-png',
                        ...
                        nargs=0,
                        default=False,
                        help='Set this parameter if you want to generate a PNG rendering of the orthophoto. (If rerunning, restart "From Orthophoto/odm_orthophoto"). '
                             'Default: %(default)s')
    ...
    parser.add_argument('--orthophoto-kmz',
                        ...
                        nargs=0,
                        default=False,
                        help='Set this parameter if you want to generate a Google Earth (KMZ) rendering of the orthophoto. (If rerunning, restart "From Orthophoto/odm_orthophoto"). '
                             'Default: %(default)s')    
    ...
    parser.add_argument('--orthophoto-compression',
                        ...
                        default='DEFLATE',
                        help='Set the compression to use for orthophotos. Can be one of: %(choices)s. (If rerunning, restart "From Orthophoto/odm_orthophoto"). Default: %(default)s')
    ...
    parser.add_argument('--orthophoto-cutline',
            ...
            'can be useful for stitching seamless mosaics with multiple overlapping orthophotos. (If rerunning, restart "From Orthophoto/odm_orthophoto"). '
            'Default: '
            '%(default)s')
    ...
    parser.add_argument('--tiles',
                    ...
                    help='Generate static tiles for orthophotos and DEMs that are '
                         'suitable for viewers like Leaflet or OpenLayers. (If rerunning, restart "From DEM/odm_dem"). '
                         'Default: %(default)s')
    ...
    parser.add_argument('--3d-tiles',
                        ...
                        nargs=0,
                        default=False,
                        help='Generate OGC 3D Tiles outputs. (If rerunning, restart "From Georeferencing/odm_georeferencing"). Default: %(default)s')
    ...
    parser.add_argument('--build-overviews',
                        ...
                        nargs=0,
                        default=False,
                        help='Build orthophoto overviews for faster display in programs such as QGIS. (If rerunning, restart "From Orthophoto/odm_orthophoto"). Default: %(default)s')

    parser.add_argument('--cog',
                        action=StoreTrue,
                        nargs=0,
                        default=False,
                        help='Create Cloud-Optimized GeoTIFFs instead of normal GeoTIFFs. (If rerunning, restart "From Orthophoto/odm_orthophoto"). Default: %(default)s')
    ...
    parser.add_argument('--pc-rectify',
                    ...
                    help=('Perform ground rectification on the point cloud. This means that wrongly classified ground '
                          'points will be re-classified and gaps will be filled. Useful for generating DTMs. (If rerunning, restart "From DEM/odm_dem"). '
                          'Default: %(default)s'))
