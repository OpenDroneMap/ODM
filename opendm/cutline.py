...
def compute_cutline(orthophoto_file, crop_area_file, destination, max_concurrency=1, scale=1):
    if io.file_exists(orthophoto_file) and io.file_exists(crop_area_file):
        ...
        def compute_linestrings(direction):
            ...
            if direction == 'vertical':
                lines = [((i, 0), (i, height - 1))
                         for i in range(line_hor_offset, width - line_hor_offset, line_hor_offset)]
                points = []
                ...
                lines = [((0, j), (width - 1, j))
                         for j in range(line_ver_offset, height - line_ver_offset, line_ver_offset)]
                points = []
        ...
        log.ODM_WARNING("We've been asked to compute cutline, but either %s or %s is missing. Skipping..."
